const CACHE = 'pg-guia-v1.1.1';
const ASSETS = [
  './','./index.html?v=1.1.1','./styles.css?v=1.1.1','./data.js?v=1.1.1','./store-media.js?v=1.1.1','./app.js?v=1.1.1','./manifest.webmanifest',
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate' || isAppAsset) {
    event.respondWith(fetch(event.request).then(response => {
      ...
      return response;
    }).catch(() => caches.match(event.request).then(cached => cached || caches.match('./index.html?v=1.1.1'))));
    return;
  }
